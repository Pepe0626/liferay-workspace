...
<s:set scope="request" var="countryId" value="countryId"/> 
<%int countryId = (Integer)(request.getAttribute("countryId")); %>
<s:set scope="request" var="zoneArray" value="zoneArray"/> 
<%com.konakart.appif.ZoneIf[] zoneArray = (com.konakart.appif.ZoneIf[])(request.getAttribute("zoneArray")); %>
<s:set scope="request" var="register" value="register"/> 
...
<script type="text/javascript">
	$(function() {
		...
		$('#datepicker').datepicker({changeMonth: true, changeYear: true, dateFormat: '<%=kkEng.getMsg("datepicker.date.format")%>', yearRange: "-120:-10", minDate: '-120y', maxDate: '-10y'});
				...
				<%if (register) { %>
					<h1 id="page-title"><kk:msg  key="b2bregistration.body.title"/></h1>
				<% } else { %>
					<h1 id="page-title"><kk:msg  key="b2edit.body.title"/></h1>
				<% } %>
	    		...
	    		<div class="content-area rounded-corners">
		    		<div>
						...
						<%if (register) { %>
							<form action="B2BCustomerRegistrationSubmit.action" id="form1" method="post">
						<% } else { %>
							<form action="B2BEditCustomerSubmit.action" id="form1" method="post">
						<% } %>
			    			<input type="hidden" value="<%=kkEng.getXsrfToken()%>" name="xsrf_token"/>
			    			<input type="hidden" value="<s:property value="editCustId" />" id="editCustId" name="editCustId"/>
			    			<input type="hidden" value="<s:property value="emailAddr" />" id="oldEmailAddr" name="oldEmailAddr"/>
							<div class="form-section">
								...
								<div class="form-section-fields">
									...
									<%if (!register) { %>
										<div class="form-input">
											<label><kk:msg  key="b2bregistration.body.password"/></label>
											<input type="password" name="optionalPassword" id="optionalPassword" autocomplete="off" value=""/>
											<span class="validation-msg"></span>
										</div>	
									<% } %>				
								</div>
								...
								<div class="form-section-fields">
									...
									<%if (kkEng.getConfigAsBoolean("ACCOUNT_STATE",false)) { %>
												...
												<select name="state" class="state" id="state">
													...
													<% for (int i = 0; i < zoneArray.length; i++){ %>
														...
														<%if (state != null && state.equals(zone.getZoneName())){ %>
															<option  value="<%=zone.getZoneName()+"::"+zone.getZoneId()%>" selected="selected"><%=zone.getZoneName()%></option>
														<% } else { %>
															<option  value="<%=zone.getZoneName()+"::"+zone.getZoneId()%>"><%=zone.getZoneName()%></option>
														<% } %>
													<% } %>
												</select>
												<span class="required-icon required-blue"></span>
										...
										<% } else {%>
											<div class="form-input">
												<label><kk:msg  key="register.customer.body.state"/></label>
												<input type="text" id="state" name="state" value="<s:property value="state" />"/>
												<span class="required-icon required-blue"></span>
												<span class="validation-msg"></span>
											</div>
										<% } %>
