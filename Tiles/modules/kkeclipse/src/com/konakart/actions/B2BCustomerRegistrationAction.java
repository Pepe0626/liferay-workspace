 ...
 */
public class B2BCustomerRegistrationAction extends B2BBaseAction
{
    ...
    public String execute()
    {
        ...
        try
        {
            ...
            // Set the address attributes with those of the primary address of the logged in
            // customer
            CustomerIf currentCust = kkAppEng.getCustomerMgr().getCurrentCustomer();
            if (currentCust != null && currentCust.getAddresses() != null
                    && currentCust.getAddresses().length > 0 && countryChange == 0)
            {
                AddressIf addr = currentCust.getAddresses()[0];
                ...
                telephoneNumber = noNull(currentCust.getTelephoneNumber());
                telephoneNumber1 = noNull(currentCust.getTelephoneNumber1());
                faxNumber = noNull(currentCust.getFaxNumber());
            }
            zoneArray = kkAppEng.getEng().getZonesPerCountry(countryId);

            kkAppEng.getNav().set(kkAppEng.getMsg("header.b2bcustomer.registration"), request);
