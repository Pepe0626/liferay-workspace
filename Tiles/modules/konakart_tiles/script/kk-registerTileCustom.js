...
kk.addRegisterTileEventHandlers = function() {
	...
	$('#kk-register-form').submit(function(e) {
		...
		if (val) {
			...
			kkEng.registerCustomerCustom(cr, function(result, textStatus, jqXHR) {
 ...
 */
kkEng.registerCustomerCustom=function(custReg,callback,context,eng){
	var parmArray = new Array();
	var index = 0;
	if (custReg != null) {
		parmArray[index++] = "custRegJSON";
		parmArray[index++] = JSON.stringify(custReg);
	}
	var jsonParms = createJsonParmString("customerRegistrationSubmit",parmArray,eng);
